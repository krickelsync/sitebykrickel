    ...
    >
      ...
      >
        {[...reviews, ...reviews].map((r, i) => (
          <div key={i} className="glass-card p-5 shrink-0">
            <div className="flex items-center gap-3 mb-3">
              <div className={`w-10 h-10 rounded-full ${r.avatarColor ?? "bg-primary"} flex items-center justify-center text-white text-xs font-bold`}>
                {r.initials ?? r.name.slice(0, 2).toUpperCase()}
              </div>
              <div>
                <p className="font-display font-bold text-sm">{r.name}</p>
                {r.rating ? (
                  <p className="text-xs text-primary">{"★".repeat(r.rating)}{"☆".repeat(5 - r.rating)}</p>
                ) : null}
              </div>
            </div>
