        ...
        <div className="max-w-5xl mx-auto grid md:grid-cols-2 gap-6 md:gap-8">
          ...
          >
            ...
            <div className="text-center mb-6 pb-6 border-b border-border/50">
              <span className="font-display text-sm uppercase tracking-widest text-muted-foreground">Standard</span>
              <div className="mt-2">
                <span className="font-display text-4xl md:text-5xl font-bold">$99</span>
              </div>
              <p className="font-mono text-xs text-muted-foreground mt-2">Perfect for starters</p>
            </div>
            ...
            <div className="flex-1 space-y-5">
              {standardFeatures.map((section, idx) => (
                <div key={idx}>
                  <span className="font-mono text-[10px] text-primary uppercase tracking-widest">{section.category}</span>
              ...
              {/* Not Included */}
              <div>
                <span className="font-mono text-[10px] text-muted-foreground/50 uppercase tracking-widest">Not Included</span>
                <ul className="mt-2 space-y-1.5">
                  {notIncludedStandard.map((item, i) => (
                    <li key={i} className="flex items-center gap-2">
                      <X size={12} className="text-muted-foreground/30 shrink-0" />
                      <span className="font-mono text-xs text-muted-foreground/40">{item}</span>
                    </li>
                  ))}
                </ul>
              </div>
            ...
            <a
              ...
              className="mt-6 block w-full text-center py-4 border border-border font-mono text-sm uppercase tracking-wider hover:bg-foreground hover:text-background transition-all duration-300"
            >
              Get Started
            </a>
          </motion.div>
          ...
          <motion.div
            ...
            className="relative premium-card rounded-xl p-6 md:p-8 flex flex-col glow-border"
          >
            {/* Promo Badge */}
            <div className="absolute -top-3 left-1/2 -translate-x-1/2 z-10">
              <span className="inline-flex items-center gap-1.5 px-4 py-1 bg-primary text-primary-foreground font-mono text-[10px] uppercase tracking-wider rounded-full">
            ...
            <div className="text-center mb-6 pb-6 border-b border-primary/20">
              <span className="font-display text-sm uppercase tracking-widest text-primary">Ultimate Premium</span>
              <div className="mt-2">
                <span className="font-display text-4xl md:text-5xl font-bold text-primary glow-text">$199</span>
              </div>
              <p className="font-mono text-xs text-muted-foreground mt-2">For high-end brands</p>
            </div>

            {/* Features */}
            <div className="flex-1 space-y-5 max-h-[500px] overflow-y-auto pr-2 scrollbar-thin">
                  ...
                  <ul className="mt-2 space-y-2">
                    {section.items.map((item, i) => (
                      <li key={i} className="flex items-start gap-2">
                        ...
                        {'value' in item && item.value && (
                          <Sparkles size={10} className="text-primary shrink-0 ml-auto" />
                        )}
                      </li>
                    ))}
                  </ul>
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="group mt-6 relative block w-full text-center bg-primary text-primary-foreground py-4 font-mono text-sm uppercase tracking-wider overflow-hidden transition-all duration-300 hover:glow-box"
            >
